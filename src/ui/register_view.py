...
from services.quiz_service import QuizService, UsernameExistsError
class RegisterView:

    def __init__(self, root, backbutton_command, togamebut_command):
        self._root = root
        self._frame = None
    ...
    def _register_handler(self):
        ...
        try:
            QuizService.register(username, password)
    ...
    def _initialize(self):
        ...
        header = ttk.Label(master=self._frame, text="tähän tuleem register")
        backbutton = ttk.Button(master=self._frame, text="Back", command=self._backbutton_command)
        togamebut = ttk.Button(master=self._frame, text="Register/play", command=self._togamebut_command)
